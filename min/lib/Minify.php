...
class Minify {
    ...
    protected static function _combineMinify()
    {
		...
		if($type != self::TYPE_CSS) {
			// minify each source (other than CSS) with its own options and minifier, then combine.
			...
			// Combine sources first, minify afterwards. Enables shared LESS functions.
			foreach (self::$_controller->sources as $source) {
				$pieces[] = $source->getContent();
			}
			self::$_controller->loadMinifier($defaultMinifier);
			$content = call_user_func($defaultMinifier, implode($implodeSeparator, $pieces), $defaultOptions);
		}
